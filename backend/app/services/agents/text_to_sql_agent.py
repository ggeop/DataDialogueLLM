...
class TextToSQLAgent:
    ...
    def generate(self, question: str) -> Tuple[str, Optional[list], Optional[str]]:
        ...
        for attempt in range(1, self._max_retries + 1):
            temperature = (
                self._initial_temperature + (attempt - 1) * self._temperature_increase
            )
    ...
    def _generate_sql(
        self, question: str, temperature: float, previous_error: str = None
    ) -> QueryResult:
        """
        ...
        if previous_error is None:
            prompt = SQL_GENERATION_TEMPLATE.format(
                db_type=self._database.db_type, schema=schema, question=question
            )
        else:
            prompt = SQL_CORRECTION_TEMPLATE.format(
                error=previous_error, db_type=self._database.db_type, question=question
            )
